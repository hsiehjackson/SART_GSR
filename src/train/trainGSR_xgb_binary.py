...
# 24
#participant_list = np.array([6,7,8,9,13,14,16,17,18,19,20,21,25,26,28,29,31,32,34,40,41,42,43,49])

fold = len(participant_list)
uselabel = ['target','target response']
infile = argv[1]
outfilename = argv[2]
print(infile)
#outfilename = 'wrist'
...
def main():
	os.makedirs("GSR_Result/{}/2_class/acc/".format(outfilename), exist_ok=True)
	os.makedirs("GSR_Result/{}/2_class/val/".format(outfilename), exist_ok=True)
	os.makedirs("GSR_Result/{}/2_class/ft/".format(outfilename), exist_ok=True)
	os.makedirs("GSR_Result/{}/2_class/importance/".format(outfilename),exist_ok=True)
...
def trainTask(task,outfile):
	...
	for num, val in enumerate(participant_list):
		...
		tn_acc, tn_f1, v_acc, v_f1, feat_size, feature, predict, target = train(X_train, Y_train, X_val, Y_val, 
																		  val, threshold , name, outfile[:-4], feature, outfilename)
	...
	"===========getresult==========="
	folder = 'GSR_Result/{}/2_class/val/'.format(outfilename)+outfile[:-4]+'/'
	...
	writefile('GSR_Result/{}/2_class/acc/'.format(outfilename)+outfile,accuracy,'w')
	writefeat('GSR_Result/{}/2_class/ft/'.format(outfilename)+outfile,best_feature,vote,'w')
